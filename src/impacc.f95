...
subroutine imp_acc_fast(truefn, imputefn, nSNPs, nAnimals, NAval, standardized, means, vars, rowcors, matcor, colcors, rowID)
  implicit none
  ...
  integer, parameter :: dp = selected_real_kind(15, 307)  ! double precision, 64-bit like, required for transferring to and fro R.
  ...
  real(dp), dimension(nSnps), intent(out) :: means, vars, colcors
  real(dp), dimension(nAnimals), intent(out) :: rowcors
  real(dp), intent(out) :: matcor

  !! Private variables
  integer :: stat, animalID, nLines, i, j, nn
  real(sp) :: t, t2, imp, imp2, tim, nan
  real(sp), dimension(nSNPs) :: M, S, Mold, Sold
  real(lp), dimension(nSNPs) :: genoin, true, imputed
  !! For running correlation on columns
  integer, dimension(nSNPs) :: cNA, nnn
  real(sp), dimension(nSNPs) :: cx, cy, cx2, cxy, cy2
  !! For matrix
  real(sp) :: mx, my, mx2, mxy, my2
  !! For row
  integer :: rNA
  real(sp) :: rx, ry, rx2, rxy, ry2
...
subroutine imp_acc_fast_full(truefn, imputefn, nSNPs, nAnimals, NAval, standardized, means, vars, rowcors, matcor, colcors, rowID)
  implicit none

  
  integer, parameter :: lp = selected_real_kind(15,307) ! low precision, for imputed genotypes
  integer, parameter :: sp = selected_real_kind(15,307) ! single precision, eq. 32 bit precision.
  integer, parameter :: dp = selected_real_kind(15, 307)  ! double precision, 64-bit like, required for transferring to and fro R.
...
!! This implementation solves this by storing the true matrix, so it shouldn't be too large.
