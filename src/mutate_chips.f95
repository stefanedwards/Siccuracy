...
subroutine cbindsnpsrwrapper(files, fnin, fnout, nlines, ncols, skiplines, idlength, excludeids, result, lenfmt, userfmt, int)

  integer, intent(in) :: files, nlines, skiplines, idlength, lenfmt, int
  ...
  if (int == 1) then
    call cbindsnpsint (files, fns, fnout, nlines, ncols, skiplines, idlength, excludeids, result, lenfmt, userfmt)
  else
    call cbindsnpsreal(files, fns, fnout, nlines, ncols, skiplines, idlength, excludeids, result, lenfmt, userfmt)
  endif
...
! No row ID checking.
subroutine cbindsnpsreal(files, fns, fnout, nlines, ncols, skiplines, idlength, excludeids, result, lenfmt, userfmt)
  implicit none
  ...
  do i=1,files
    ...
    fmt(i)='('//trim(adjustl(fmt0))//'F'//trim(adjustl(userfmt))//')'
...
end subroutine cbindsnpsreal

! Concatenates genotype matrices (from e.g. multiple chromosomes) into one.
! No row ID checking.
subroutine cbindsnpsint(files, fns, fnout, nlines, ncols, skiplines, idlength, excludeids, result, lenfmt, userfmt)
  implicit none
